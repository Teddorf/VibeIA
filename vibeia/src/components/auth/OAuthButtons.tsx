...
const PROVIDERS: Record<
  OAuthProvider,
  { name: string; Icon: React.FC; bgColor: string; hoverColor: string }
> = {
  github: {
    name: 'GitHub',
    Icon: GitHubIcon,
    ...
    <div className="space-y-4">
      ...
      <div className={`flex ${variant === 'compact' ? 'justify-center gap-4' : 'flex-col gap-3'}`}>
        {enabledProviders.map((provider) => {
          ...
          const textColor = provider === 'google' ? 'text-slate-200' : 'text-white';
          ...
          return (
            <button
              ...
              `}
            >
              {isCurrentLoading ? <LoadingSpinner /> : <Icon />}
