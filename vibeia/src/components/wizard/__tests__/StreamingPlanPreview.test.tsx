...
describe('StreamingPlanPreview', () => {
    ...
    it('should collapse/expand phases', async () => {
      ...
      render(
        <StreamingPlanPreview
          ...
          partialPlan={{
            phases: [
              { name: 'Phase 1', tasks: [{ name: 'Task 1', estimatedTime: 30 }], status: 'complete' },
            ],
          }}
        />
      );
  ...
  describe('Accessibility', () => {
    ...
    it('should announce progress changes to screen readers', () => {
      ...
      expect(screen.getByRole('progressbar').closest('[aria-live]') ||
             screen.getByText(/diseñando/i).closest('[aria-live]')).toBeTruthy();
    });
  });
});
