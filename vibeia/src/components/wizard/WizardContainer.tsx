...
export function WizardContainer({ existingProjectId }: WizardContainerProps) {
  ...
  const [executionInfo, setExecutionInfo] = useState<{ projectId: string; planId: string } | null>(
    null,
  );
  const [showRestorePrompt, setShowRestorePrompt] = useState(false);

  // Wizard mode management
  const { mode, setMode, hasPreference, userExperience, clearPreference } = useWizardMode();

  // Auto-save progress
  const { savedProgress, hasRestorable, updateProgress, restoreProgress, clearProgress } =
    useWizardProgress();
  ...
  if (stage === 0) {
    return (
      <div className="max-w-4xl mx-auto p-6">
        <WizardModeSelector onSelect={handleModeSelect} userExperience={userExperience} />
      </div>
    );
  }
  ...
  if (mode === 'expert' && stage === 1) {
    return (
      <div className="max-w-6xl mx-auto p-6">
        <ExpertMode onComplete={handleExpertComplete} onBack={handleBackToModeSelection} />
      </div>
    );
  }
  ...
  if (mode === 'guided' && stage === 1) {
    return (
      <div className="max-w-4xl mx-auto p-6">
        <GuidedMode onComplete={handleGuidedComplete} onBack={handleBackToModeSelection} />
      </div>
    );
  }
      ...
      <div className="max-w-2xl mx-auto p-6">
        <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-6 text-center">
          ...
          <div className="flex gap-4 justify-center">
            ...
            <Button onClick={handleRestoreProgress}>Continuar</Button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-8">
        <div className="flex justify-between text-sm text-muted-foreground mb-2">
          ...
          <div className="flex items-center gap-4">
            <button onClick={handleBackToModeSelection} className="text-xs hover:underline">
              Cambiar modo
            </button>
      ...
      {stage === 1 && <Stage1IntentDeclaration onNext={handleStageComplete} />}
