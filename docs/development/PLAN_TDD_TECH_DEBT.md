...
export class RequirePermissionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const teamParamKey =
      this.reflector.get<string>(TEAM_PARAM_KEY, context.getHandler()) || 'teamId';
    ...
    const hasPermission = await this.membersService.hasPermission(teamId, userId, permission);
...
export const projectsApi = {
  ...
  archive: async (id: string) => (await apiClient.post(`/api/projects/${id}/archive`)).data,
};
