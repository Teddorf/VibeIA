...
describe('ExecutionService', () => {
  ...
  beforeEach(async () => {
    ...
    const mockUsersService = {
      ...
      getActiveLLMApiKeys: jest.fn().mockResolvedValue({ anthropic: 'sk-test-key' }),
  ...
  describe('executePlan', () => {
    it('should throw error if plan not found', async () => {
      ...
      await expect(service.executePlan('invalid-id', mockUserId)).rejects.toThrow('Plan not found');
    });
    ...
    it('should throw error if project not found', async () => {
      ...
      await expect(service.executePlan('plan-123', mockUserId)).rejects.toThrow('Project not found');
    });
    ...
    it('should execute all automated tasks in plan', async () => {
      ...
      expect(plansService.updateStatus).toHaveBeenCalledWith('plan-123', 'in_progress', mockUserId);
    ...
    it('should continue execution after quality gates pass', async () => {
      ...
      expect(plansService.updateStatus).toHaveBeenCalledWith('plan-123', 'completed', mockUserId);
    });
  ...
  describe('getExecutionStatus', () => {
    ...
    it('should throw error if plan not found', async () => {
      ...
      await expect(service.getExecutionStatus('invalid-id', mockUserId)).rejects.toThrow('Plan not found');
    });
  });
  ...
  describe('pauseExecution', () => {
    it('should pause execution and update plan status', async () => {
      ...
      expect(plansService.updateStatus).toHaveBeenCalledWith('plan-123', 'paused', mockUserId);
    });
  });

  describe('resumeExecution', () => {
    it('should throw error if no execution state found', async () => {
      await expect(service.resumeExecution('unknown-plan', mockUserId)).rejects.toThrow('No execution state found');
    });
